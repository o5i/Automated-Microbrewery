...
void loop()
{
  ...
  if (REG[10]){
    //analogWrite(PWM_1, map(REG[1], REG[60], REG[61], 0, 65535)); // Use if change resolution "Bitshift"
    analogWrite(PWM_1, REG[1]); // Use if change resolution "Bitshift"
  } else {
    analogWrite(PWM_1, 0); // Use if change resolution "Bitshift"
  }
  if (REG[11]){
    //analogWrite(PWM_2, map(REG[2], REG[60], REG[61], 0, 65535)); // Use if change resolution "Bitshift"
    analogWrite(PWM_2, REG[2]); // Use if change resolution "Bitshift"
  } else {
    analogWrite(PWM_2, 0); // Use if change resolution "Bitshift"
  }
  //analogWrite(PWM_1, map(REG[1], 0, 65535, REG[60], REG[61])); // Use if change resolution "Bitshift"
  //analogWrite(PWM_2, map(REG[2], 0, 65535, REG[60], REG[61])); // Use if change resolution "Bitshift"
  digitalWrite(DO_1, !REG[10]);
  digitalWrite(DO_2, !REG[11]);
  digitalWrite(DO_3, !REG[12]);
  digitalWrite(DO_4, !REG[13]);
